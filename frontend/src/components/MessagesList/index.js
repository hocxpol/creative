...
import whatsBackgroundDark from "../../assets/background-whatsapp/wa-background-dark.webp";
...
const reducer = (state, action) => {
			...
			try {
				...
				if (editInfo) {
					...
					if (originalMessageId) {
						...
						if (messageIndex !== -1) {
              // Atualiza a mensagem original com todos os campos necessários
							const updatedMessage = {
								...state[messageIndex],
								body: messageToUpdate.body,
								mediaType: messageToUpdate.mediaType,
                isEdited: true,
                editedAt: messageToUpdate.createdAt || new Date().toISOString()
							};
							state[messageIndex] = updatedMessage;
							return [...state];
						}
    ...
    // Para mensagens não editadas ou deletadas, atualiza o estado
		const messageIndex = state.findIndex((m) => m.id === messageToUpdate.id);
		if (messageIndex !== -1) {
      // Cria uma nova referência do array para garantir re-render
      const newState = [...state];
      // Atualiza a mensagem mantendo os campos existentes e adicionando/atualizando os novos
      newState[messageIndex] = {
        ...state[messageIndex],
        ...messageToUpdate,
        isDeleted: messageToUpdate.isDeleted !== undefined ? messageToUpdate.isDeleted : state[messageIndex].isDeleted,
        isEdited: messageToUpdate.isEdited || state[messageIndex].isEdited,
        editedAt: messageToUpdate.isEdited ? (messageToUpdate.editedAt || new Date().toISOString()) : state[messageIndex].editedAt
      };
      return newState;
		}

    // Se a mensagem não existe no estado, adiciona ao início
    if (messageToUpdate.isDeleted) {
      return [messageToUpdate, ...state];
    }
		return [...state];
	}
...
const MessagesList = ({ ticket, ticketId, isGroup }) => {
	...
	const MAX_FILES = 30; // Limite máximo de arquivos
	const ALLOWED_TYPES = [
		'image/jpeg',
		'image/png',
		'image/gif',
	...
	useEffect(() => {
        ...
        // Force immediate re-render for deleted messages
        if (data.message.isDeleted) {
          dispatch({ type: "UPDATE_MESSAGE", payload: data.message });
        }

        if (data.ticket) {
          ticket.lastMessage = data.ticket.lastMessage;
        }
			}
		});
	...
	const renderMessageAck = (message) => {
		...
		if (!message.read && message.fromMe) {
			return <ErrorOutline fontSize="small" className={classes.ackIcons} style={{ color: '#f44336' }} />;
		}
	};
		...
		if (messagesList.length > 0) {
			const viewMessagesList = messagesList.map((message, index) => {
				...
				if (!message.fromMe) {
                    ...
                    {message.isDeleted && <span>Apagada </span>}
                    {format(parseISO(message.createdAt), "HH:mm:ss")}
									</span>
								</div>
							</div>
						</React.Fragment>
	...
	const handleUploadFiles = async (files) => {
		...
		try {
			...
			for (let i = 0; i < files.length; i++) {
				...
				formData.append("medias", fileData.description || fileData.file);
