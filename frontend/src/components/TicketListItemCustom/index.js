...
import ForceCloseTicketModal from "../ForceCloseTicketModal";
...
const TicketListItemCustom = ({ ticket, onTicketSelect, onTicketClose }) => {
  ...
  const handleCloseTicket = async (id) => {
    ...
    setLoading(true);
    try {
      // Verifica se há conexão com o WhatsApp
      const { data: whatsapp } = await api.get(`/whatsapp/${ticket.whatsappId}`);
      
      if (!whatsapp || !whatsapp.status || whatsapp.status !== "CONNECTED") {
  ...
  const handleForceClose = async () => {
    setLoading(true);
    try {
      await api.put(`/tickets/${selectedTicketId}`, {
        status: "closed",
        userId: user?.id,
        queueId: ticket?.queue?.id,
        useIntegration: false,
