...
const typebotListener = async ({
    ...
    try {
        ...
        if (body !== typebotKeywordFinish && body !== typebotKeywordRestart) {
            ...
            if (messages?.length === 0 && typebotUnknownMessage && typebotUnknownMessage !== "Invalid message. Please, try again.") { // Adicionado Glorium para evitar mensagem em branco
                ...
                for (const message of messages) {
                    if (message.type === 'text') {
                        ...
                        if (formattedText) { // Adicionado Glorium para evitar mensagem em branco
