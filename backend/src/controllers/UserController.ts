...
export const update = async (
  ...
  const { profile, companyId, id: requestUserId } = req.user;
  const { userId } = req.params;
  const userData = req.body;

  // Permitir tanto "admin" quanto "user" para atualizar
  if (!["admin", "user"].includes(profile)) {
    throw new AppError("ERR_NO_PERMISSION", 403);
  }

  // Adicionar validação para garantir que o usuário só possa editar dentro do mesmo companyId
  /*if (profile === "user" && req.user.id !== parseInt(userId)) {
    throw new AppError("ERR_NO_PERMISSION", 403);
  }*/
