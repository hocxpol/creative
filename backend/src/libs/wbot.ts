      ...
      (async () => {
        ...
        wsocket.ev.on(
          ...
          async ({ connection, lastDisconnect, qr }) => {
            ...
            if (connection === "open") {
							logger.info(`WhatsApp ${whatsapp.id} connection opened`);
							const fullNumber = jidNormalizedUser((wsocket as WASocket).user?.id);
              ...
              if (sessionIndex === -1) {
                ...
                sessions.push(wsocket);
								logger.info(`Added WhatsApp ${whatsapp.id} to sessions`);
              }
